...
from helpers import plot_constructor, initialization_models, update_output_values
...
def initialize(sp_num_breweries, sp_total_price, sp_br, sp_mead_in_wallet):
    global PLAYER, PLOT
    ...
    br = int(sp_br.value)
    mead_in_wallet = float(sp_mead_in_wallet.value)
...
btn_initialize.on_click(partial(initialize, sp_num_breweries, sp_total_price, sp_init_br, sp_init_mead))


# VISUALIZATION
# create data sources to feed data into
data_source = ColumnDataSource(data={"day": [], "unclaimed_mead": [], "mead_in_wallet": []})
...
sel_strats = Select(title="Strategy:", value="hodl", options=["hodl", "compound", "claim daily"])
...
full_layout = row(in_out_layout, PLOT)
